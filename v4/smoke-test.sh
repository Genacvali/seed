#!/bin/bash
# SEED Agent v4 - Smoke Test Script
...
ALERT_RESPONSE=$(curl -s -X POST "${BASE_URL}/alert" \
  ...
  -d "{\"alerts\":[{\"labels\":{\"alertname\":\"SmokeTest\",\"instance\":\"${HOST}\",\"severity\":\"warning\"},\"annotations\":{\"summary\":\"SEED Agent v4 smoke test\"},\"status\":\"firing\"}]}")
